...
## 1-1. 이 프로젝트가 실제로 해결하려는 것

### 사용자 입장에서의 문제
- "탐지됐다"만으로는 의사결정이 어렵다 (왜 그렇게 판단했는지 근거가 필요)
- 영상량 증가로 판독 부담이 커지고, 작은 병변을 놓칠 위험이 있다
- AI가 근거 없이 길게 설명하면 오히려 신뢰가 떨어진다

### 우리가 잡은 해결 방향
- **검출(YOLO)**: 병변 위치/신뢰도 정량 제공
- **해석(VLM)**: 영상의 정성적 소견 보강
- **근거(RAG)**: 문헌 기반 근거를 함께 제시
- **안전가드**: 근거가 부족하면 추측 답변 대신 "근거 없음" 반환

### 한 줄 가치 제안
> "의료 영상 AI를 정확도 데모에서 끝내지 않고, 실제 사용자가 이해하고 검증할 수 있는 형태로 바꾼 프로젝트"
