...
같은 YOLOv8n-seg 모델, 같은 학습 조건으로 두 데이터셋을 학습했으나 폴립(0.942) 대비 치과 X-ray(0.344)가 2.7배 낮았습니다.

원인을 3가지로 분석했습니다:

| 원인 | 내용 |
|------|------|
| 데이터 부족 | Kvasir ~1,000장 vs DENTEX ~175장 — 탐지 모델 권장 기준(500장) 미달 |
| 이미지 저대비 | 파노라마 X-ray 흑백 특성 → 병변 경계 식별 어려움 |
| 클래스 유사성 | 4개 병변 외형이 유사 → 모델 혼동 발생 |

원인 기반으로 3단계 개선 실험을 설계해 반복 검증했습니다:
- 이미지 크기 640 → 768 / optimizer SGD → AdamW / lr 0.01 → 0.003
- patience 15 → 25 / Mosaic·회전·이동 증강 추가
- baseline / recall_boost / balance_recover 3단계 비교

수치 개선은 제한적이었으나, "왜 안 됐는지"를 데이터 기반으로 설명할 수 있는 상태를 확보했습니다. 동일 구조에서 데이터 규모 확보 시 성능 개선 가능성을 구체화했습니다.

상세 분석 → [docs/DENTEX_ANALYSIS.md](docs/DENTEX_ANALYSIS.md)
