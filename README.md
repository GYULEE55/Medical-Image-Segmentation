...
> 이 수치는 현재 벡터스토어(38개 문서)를 기준으로 한 추정치입니다.
...
│
├── 📚 rag/                  # RAG 파이프라인
│   ├── chain.py             # LangChain LCEL + no-evidence 가드
...
| **RAG** | LangChain LCEL, ChromaDB, BGE-M3 임베딩 |
...
### 가장 어려웠던 것
**1,206줄 모놀리식 `app.py`를 라우터 기반으로 분리**하면서 기존 36개 테스트를 모두 통과시키는 작업이었습니다.  
`app.state` 패턴으로 YOLO 모델 객체와 `async_jobs` 딕셔너리를 안전하게 공유했습니다.

### 배운 것
- 의료 AI에서 **"모른다고 말하는 것"** 이 **"틀린 답을 말하는 것"** 보다 훨씬 중요하다
- 성능이 낮게 나왔을 때 원인 분석과 문서화 자체가 엔지니어링 역량을 보여준다
- RAG의 관련성 필터링 임계값(min_relevance=0.2) 하나가 응답 품질을 크게 바꾼다
...
*의료공학과 + AI 엔지니어링을 결합한 포트폴리오 프로젝트*  
*Kvasir-SEG (위장 폴립) + DENTEX (치과 X-ray) 멀티모달 의료 영상 AI*
